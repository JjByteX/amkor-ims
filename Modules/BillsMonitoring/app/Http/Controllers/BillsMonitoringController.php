...
class BillsMonitoringController extends Controller
{
    ...
    {
        $role = $request->user()?->getRoleNames()->first();
        if (! in_array($role, ['general_manager', 'disbursement_officer', 'hr_admin_officer'], true)) abort(403);
        if ($bill->isApproved()) return back()->with('flash', ['type' => 'error', 'message' => 'Cannot delete an approved record.']);
    ...
    {
        if (! $this->canCheck($request)) abort(403, 'Only Admin Auditor or General Manager can check bills.');
        if ($bill->checked_at) return back()->with('flash', ['type' => 'warning', 'message' => 'Already checked.']);
    ...
    {
        if (! $this->canApprove($request)) abort(403, 'Only the General Manager can approve bills.');
        if (! $bill->checked_at) return back()->with('flash', ['type' => 'error', 'message' => 'Bill must be checked before approval.']);
        if ($bill->approved_at) return back()->with('flash', ['type' => 'warning', 'message' => 'Already approved.']);
    ...
    {
        $this->requirePreparer($request);
        if (! $bill->approved_at) return back()->with('flash', ['type' => 'error', 'message' => 'Bill must be approved before marking as paid.']);
    ...
    {
        if (! $this->canPrepare($request)) abort(403, 'Only HR & Admin Officer or Disbursement Officer can manage bills.');
    }
}
