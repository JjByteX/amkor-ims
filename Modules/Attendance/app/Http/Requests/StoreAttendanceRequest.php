...
class StoreAttendanceRequest extends FormRequest
{
    ...
    {
        // Trim seconds from time inputs (form sends HH:MM, DB stores HH:MM:SS — handled at model level)
        if ($this->time_in) {
            $this->merge(['time_in' => substr($this->time_in, 0, 5)]);
        }
        if ($this->time_out) {
            $this->merge(['time_out' => substr($this->time_out, 0, 5)]);
        }
    }
}
